...
<body>
    ...
    <section class="cWriteSection">
        <div class="schFrm">
            ...
            <form action="Store_Posting_upt" method="post" enctype="multipart/form-data" id="Lasubmit">
                <input type="hidden" value="${insSle.prodloc}" id="prodloc" name="prodloc">
                ...
                <input type="hidden" value="${insSle.dcprice}" id="dcprice" name="dcprice">
                <input type="hidden" value="${insSle.country}" name="country">
                <!-- 상품 판매 작성자 정보 수정바람 -->
                <input type="hidden" value="${user.userid}" id="writer" name="writer">
                <input type="hidden" value="${insSle.prodkey}" id="prodkey" name="prodkey">
                <input type="hidden" value="${insSle.titleimage}" id="oldimg01" name="oldimg01">
                <input type="hidden" value="${insSle.detailimage}" id="oldimg02" name="oldimg02">
...
var selectedValues = "${insSle.prodloc}"; // 지역 체크

$("[name=loc]").each(function() {
    var value = $(this).val();
...
// 요일 체크
var selectedValues = "${insSle.dcday}"; // 할인 요일
$("[name=saleDay]").each(function() {
    var value = $(this).val();
    if (selectedValues.includes(value)) {
        $(this).prop("checked", true);
    }
...
// 달력
   //주어진 날짜 문자열
   var dateString = "${insSle.startdate}";
   ...
   var dateStringend = "${insSle.enddate}";
   // 주어진 날짜 문자열을 변환
   var parts = dateStringend.split(' ');
   var year = parts[5];
   var month = monthMap[parts[1]];
   var day = parts[2];
   
   var formattedDateend = year + '-' + month + '-' + day;
   console.log(formattedDateend);
   $("#enddate").val(formattedDateend)
// 달력
// 퍼센트
   $(".saleDrop").val(${insSle.dcprice})
// 퍼센트
// 제목
   $("[name=prodtitle]").val("${insSle.prodtitle}")
// 제목
//배경 이미지
...
$('.uploadedBtn')[1].classList.add("uploaded");
   ...
   $(".infoImg").empty();
    var img1 = $('<img>');
    var img2 = $('<img>');
    img1.attr('src', "./source/storeImg/${insSle.titleimage}");
   $(".infoImg").append(img1);
//배경 이미지
//상세글
   $("[name=prodcontent]").val(`${insSle.prodcontent}`)
//상세글
//설명 이미지
   $(".bgImg").empty();
    img2.attr('src', "./source/storeImg/${insSle.detailimage}");
   $(".bgImg").append(img2);
//설명 이미지
// 가격
   $("[name=price]").val(${insSle.price})
// 가격
    ...
    $(document).ready(function () {
        ...
        var addName = "heoyoun38" // 작성자 정보 (수정바람)
        var selectedCountry = "";
        var sale = 0;
        var title = "";
        var context = "";
      ...
      $(".writer1").text(addName);
        ...
        $("#dcdayBtn").click(function () {
            ...
            $(".daySelecAll").find("[name=saleDay]:checked").each(function () {
        ...
        $(".saleDrop").change(function () {
	        ...
	        $(".submitBtn").click(function () {
	        	console.log(dateClick01)
	        	console.log(dcdayBtn02)
